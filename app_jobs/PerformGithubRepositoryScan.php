...
class PerformGithubRepositoryScan implements ShouldQueue
{
    ...
    */
    ...
    public function handle(
        ...
        Log::info("TITAN: Initial Token Check", ['has_token' => !empty($resolvedToken)]);
        
        if (empty($resolvedToken) && method_exists($this->project, 'getGithubToken')) {
            $resolvedToken = $this->project->getGithubToken();
            if (!empty($resolvedToken)) Log::info("TITAN: Token resolved from project decryption.");
        }

        // TITAN V2: System Fallback for Private Repos
        if (empty($resolvedToken)) {
            $resolvedToken = env('GITHUB_SYSTEM_TOKEN');
            if ($resolvedToken) Log::info("TITAN: Using System Fallback Token for repo scan.");
        }
        ...
        // 2. DNA RECONNAISSANCE (Dependencies) via API (Optional, but safe to keep for manifest checks)
            ...
            try {
                // A. CLONE
                if ($vaultAsset) {
                    Log::info("TITAN: Dispatching Clone Progress", ['asset_id' => $vaultAsset->id]);
                    AuditProgressUpdated::dispatch($vaultAsset, 'Git: Cloning Repository (This may take a moment)...', 10);
                }
